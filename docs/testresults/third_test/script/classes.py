...
##        assert(self.findOverlap(building1, building2) == False), 'Buildings overlap'
    ...
    def calcTotalPrice(self):
        ...
        for building in self.buildings:
            DistList.append(self.findShortestDist(building))
            #print building.name
        ...
        return totalPrice, totalExtraVrijstand, DistList
...
##                print len(self.buildings)
    ...
    def updateGrid(self, simulations):
        ...
        for simulation in range(simulations):
            ...
            totalvals = [distance, totalprijsverb]
            completevals = totalvals + totalPrice[2]

            writer.writerow(completevals)
...
##        anim.updateAnimation(best_buildings, best_prijsverb)
        ...
        return best_buildings, worst_buildings, int(best_prijsverb), distance, int(worst_prijs)
...
#    grid = Grid(120, 160, 2)
    ...
    header = ['Distance', 'Prijs']
    maisons = 3 * ['maison']
    bungalows = 5 * ['bungalow']
    eengezins = 12 * ['eengezins']


    file = open('results.csv', 'wb+')
    writer = csv.writer(file)
    writer.writerow(header + maisons + bungalows + eengezins)
    ...
    GridVisualisation(120, 160, testcase[0], testcase[2]) # Beste
    GridVisualisation(120, 160, testcase[1], testcase[4]) # Slechtste

    #cProfile.run('grid.updateGrid(1)')
