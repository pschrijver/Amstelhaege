        ...
        for i in buildings:
            ...
            if i.name == 'maison':
                  self.w.create_polygon(points, 
                            fill='black')
                  self.w.create_text((x1+22,y1-23), fill="white", text=i.index, font=("arial",8))
            if i.name == 'eengezinswoning':
                  self.w.create_polygon(points, 
                            fill='red')
                  self.w.create_text((x1+17,y1-17), fill="white", text=i.index, font=("arial",8))
            if i.name == 'bungalow':
                  self.w.create_polygon(points,
                            fill='blue')
                  self.w.create_text((x1+19,y1-17), fill="white", text=i.index, font=("arial",8))
...
class Grid(object):
    ...
    def findOverlap2(self, building):
        ...
        diagonal = math.sqrt(building.width**2 + building.depth**25) + math.sqrt(11**2 + 10.5**2)
        for neighbor in self.buildings:
            # Calculate distance for the building
            building.neighbors[neighbor] = self.findDistance(building, neighbor)
    ...
    def findShortestDist(self, building):
        ...
        for neighbor in self.buildings:
            ...
            if building != neighbor and dist < maxDist:
                ...
                # Saves all the distances in a dictionary
                building.neighbors[neighbor] = newDist
...
##        if extravrijstand != extravrijstand2:
##            print building.index
##            print extravrijstand, extravrijstand2
...
##            priceAndVrijstand2 = self.calcPrice2(building)
            totalPrice += priceAndVrijstand[0]
     
            totalExtraVrijstand += priceAndVrijstand[1]
##            totalExtraVrijstand2 += priceAndVrijstand[2]
...
##                print len(self.buildings)
    ...
    def randomPlacements2(self):
        ...
        while noConfiguration:
            ...
            while i < self.aantalhuizen and randomTries < 1000:
                if i < .15 * self.aantalhuizen:
                    ...
                    building = Maison(ran_x, ran_y, ran_angle, self.width, self.depth, index)
                    
                elif i < .4 * self.aantalhuizen:
                    ran_x = random.random() * (self.width )
                    ran_y = random.random() * (self.depth )
                    ran_angle = 0#random.randrange(0,360)
                    building = Bungalow(ran_x, ran_y, ran_angle, self.width, self.depth, index)
       
                else:
                    ran_x = random.random() * (self.width )
                    ran_y = random.random() * (self.depth )
                    ran_angle = 0#random.randrange(0,360)
                    building = EengezinsWoning(ran_x, ran_y, ran_angle, self.width, self.depth, index)
        ...
        # Saves a dict with all distances (neighbors)
        # Saves the minimum of that dict (shortestDist)
        # Adds the building to the hasAsNearest list
        for building in self.buildings:
            for neighbor in self.buildings:
                if building != neighbor:
                    ...
                    building.neighbors[neighbor] = self.findDistance(building, neighbor)
##                    print 'the distance'
##                    print building.index, neighbor.index, building.neighbors[neighbor]

            # Saves the nearestneighbor
            nearestNeighbor = min(building.neighbors, key=building.neighbors.get)
            nearestNeighbor.hasAsNearest.append(building)
            # Saves the shortestDistance of that building.
            building.shortestDist = min(building.neighbors.itervalues())
...
##        anim.updateAnimation(best_buildings, best_prijsverb)
    ...
    def newRandomPosSA2(self, building, previousPrice, noChange):
        currentX = building.x
        currentY = building.y
        translate = False
        newPos = random.randrange(1, 5)
        #
        ...
        building.newPosition(newX, newY)
        building.shortestDist = min(building.neighbors, key=building.neighbors.get)
        # If position valid calculate the new price
        if not self.findOverlap2(building):
            translate = True
            # Loops through all the neighbors of the moved building
            for neighbor in self.buildings:
                if building != neighbor:
                    # Finds the new distance to all those buildings
                    distance = self.findDistance(neighbor, building)
                    # Saves the new distance to those buildings
##                    building.neighbors[neighbor] = distance
                    neighbor.neighbors[building] = distance
                    if distance < neighbor.shortestDist:
                        neighbor.shortestDist = distance

            for nearestNeighbor in building.hasAsNearest:
                building.hasAsNearest.remove(nearestNeighbor)

                for neighbors in self.buildings:
                    distance = self.findDistance(nearestNeighbor, neighbors)
                    nearestNeighbor.neighbors[neighbors] = distance
                del nearestNeighbor.neighbors[nearestNeighbor]

                nearestNeighbor.shortestDist = min(nearestNeighbor.neighbors, key=nearestNeighbor.neighbors.get)

##                nearestNeighbor.hasAsNearest.append(nearestNeighbor.shortestDist)
                nearestNeighbor.shortestDist.hasAsNearest.append(nearestNeighbor)
                nearestNeighbor.shortestDist = nearestNeighbor.neighbors[nearestNeighbor.shortestDist]

            # Finds its own shortestDistance
            del building.neighbors[building]
            building.shortestDist = min(building.neighbors, key=building.neighbors.get)
            building.shortestDist = building.neighbors[building.shortestDist]
...
####        # change back to previous configuration
        ...
        return newPrice, translate

           
    

def translatingRandomSampleSA2(aantalhuizen, gridWidth, gridDepth, step):
    """
    ...
    while noChange < 3000:
##        print 'TOTAL 1', str(datetime.now())
##        print 
##        print 'grid', str(datetime.now())
        old_grid = copy.deepcopy(grid)
##        print str(datetime.now())

        # Makes animation for every 1000th iteration
        if i%100==0:
            print i, previousPrice, noChange
            anim.emptyAnimation(grid.buildings)
            anim.updateAnimation(grid.buildings, newPrice)

        # Choose random building
        building = grid.buildings[random.randrange(0, aantalhuizen)]
##        print
##        print 'price', str(datetime.now())
        newPrice = grid.newRandomPosSA2(building, previousPrice, noChange)
##        print str(datetime.now())
        priceDif = newPrice[0] - previousPrice[0]
        ...
        if newPrice[1]:
            if newPrice >= previousPrice:
                accept = math.exp((newPrice[0] - previousPrice[0]) * i / lifetime)
                if random.random() < accept or newPrice == 0:
                    previousPrice = newPrice
                else:
##                    print
...
##            print str(datetime.now())
        i += 1
##        print 'TOTAL 2', str(datetime.now())
    ...
    for building in grid.buildings:
        print
        n = []
        for nearest in building.hasAsNearest:
            n.append(nearest.index)
        print 'building number: ', building.index, 'has : ', building.shortestDist
        print 'Has as Nearest= ', n
...
def combinationRandomSample2SA(aantalhuizen, gridWidth, gridDepth, step):
    ...
    while noChange < 5000:
        ...
        if random.random() > 0.8:    
            # Choose random building
            building1 = grid.buildings[random.randrange(0, aantalhuizen)]
            building2 = grid.buildings[random.randrasnge(0, aantalhuizen)]
...
class EengezinsWoning(Building):
    def __init__(self, x, y, angle, gridWidth, gridDepth, index):
...
##
    ...
    a = translatingRandomSampleSA2(20, 120, 160, 0.5)
